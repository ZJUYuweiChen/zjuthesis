...
\par 项目与任务接口构成任务管理域的数据骨架，是用户规划与追踪进度的核心载体。每种资源均同时提供单条与批量两种变体，批量变体的路径在原路径基础上追加 \texttt{/batch} 后缀，请求体携带数组形式的操作载荷，由后端在事务内统一执行，便于 AI 智能体在一次任务拆解中一次性写入多条任务。任务记录标题、截止日期、优先级与完成状态等字段，用户可通过状态筛选快速定位当前关注的工作项。任务隶属于具体项目，因此创建任务和按项目分页查询任务的路径以项目标识符为父路径段，而对任务自身的更新和删除则使用扁平路径 \texttt{/task/\{id\}}。表 \ref{tab:api-project} 汇总了该模块的接口。
...
\par 测试共执行用例123个，整体通过率达到100\%，满足预设目标。普通接口测试验证了项目视图从创建到删除的完整生命周期以及批量操作的事务一致性，递归比较结果与预期完全一致，表明数据持久化和序列化逻辑正确。智能体测试验证了智能路由、历史记忆和上下文连续性等核心智能体功能，对话状态在多次交互间保持一致，工具调用结果正确。云服务集成测试验证了短信发送和登录流程在真实网络环境下的连通性和正确性，错误验证码被正确拒绝，表明与阿里云短信服务的集成稳定可靠。
...
\par 从接口响应维度来看，核心端点 P50 延迟 7.991 ms、P95 延迟 11.129 ms，系统响应良好；理论最大吞吐量约 25,026.9 RPS，并发承载潜力较高。

\par 从数据库与 JVM 维度来看，列表查询延迟比例 0.45x，不存在显著 N+1 问题；Flyway 迁移 156 ms，初始化高效；JWT 与 DB 开销约 7.576 ms，提示词平均渲染速度 120.69 tokens/μs。

\par 从并发稳定性维度来看，8线程并发更新全部成功且无异常，数据一致性和事务隔离良好；线程池未饱和，未触及资源瓶颈。

\par 从智能体维度来看，首字节时间约 61.113 ms，处于可接受范围；流式输出间隔约 18.505 ms，节奏稳定；降级快速失败约 20.040 ms，响应迅速；状态持久化约 56.093 ms，Redis 写入良好。

\par 从缓存维度来看，Redis 缓存对语音服务令牌加速显著，延迟从 357.111 ms 降至 4.969 ms，加速比 71.87 倍，缓存策略对高频访问接口提升显著。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{4cm}p{4cm}}
    ...
    Prompt 平均渲染速度 & 120.69 tokens/μs \\
    \hline
    ...
    降级快速失败延迟 & 20.040 ms \\
    对话状态持久化延迟 & 56.093 ms \\
    Token 吞吐量 & 264,625.5 tokens/sec \\
    \hline
